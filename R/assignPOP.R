...
g.n9 <- g.stratified[,,which(getNumInd(g.stratified, by.strata = TRUE)$num.ind > 8)]
pws.struct <- pairwiseSummary(pairwiseTest(g.n9, nrep = 10))

strata2keep <- c("CentAm-CA.OR.WA", "MnMx-CA.OR.WA", "HI-WA.SBC")
...
accuracy.plot( accuMC, pop='MnMx-CA.OR.WA') +
